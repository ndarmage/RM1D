...
Fig.~\ref{fig:slab-fluxes} shows the fast and thermal fluxes calculated by diffusion, CMFD, pCMFD and S$_16$ of the left half slab. The abscissa shows values of the group-dependent neutron mean free path (mfp) $\tau$.
...
One or two mpf's from the boundary towards the center of the slab are enough for diffusion, to match well the reference S$_16$ results. On the other hand, closer to the boundary, the Ronen method shows better results. The flux deviations of the left half slab, are shown in Fig.~\ref{fig:flx_err_homg}, calculated by $\Delta\keff = \left(\keff^{-1}-k_{\text{ref}}^{-1}\right)\times 10^5$ pcm.
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.9\linewidth]{Tomatis2011_rho_I400_RMitr250.pdf}	
...
\begin{figure}[!htbp]
	...
	\caption{The Ronen method correction factors ($\delta D$), calculated by CMFD code, and ($\delta D^+, \delta D^-$), calculated by pCMFD code. Results shown here after 250 Ronen iterations.}
