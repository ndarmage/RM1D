...
\\ \\
\revised{This is correct. Ronen did not elaborate on this issue in his technical note from 2004. Theoretically, both the numerator and the denominator should vanish at the same location, since both are accurate expressions for the neutron current at this location (where the flux gradiaent is weak). We did not try to find the analytic limit of this expression for $D$, however we assume that this ratio between two expressions, which vanish at the same location, approaches some constant value.\\ \\ Numerically, Tomatis and Dall'Osso 2011 chose to deal with this potential singularity by using the well-known drift terms, which serve as a \emph{numerical feature}. The (more accuarte) surface current $J_{tr}$, calculated by intergal expression, is written as the sum of the surface current calculated by diffusion, $J_D$, plus a correction $\delta J$. Hence, when the current vanishes, the correction $\delta J$ valishes. Moreover, the correction is writtten as in Eq. (21), avoiding the potential divergence resulting from division by zero.}
\\ \\
% --------------------------------------------------------------------------------------
...
\\ \\
\revised{An entire benchark was added to the results section. The chosen benchmark is a one-dimensional highly heterogeneous core representing a BWR core. The sensitivity of the method to the slab width, especially at the boundaries, will be studied in future work, along with additional effects emanate from the boundaries conditions.}
\\ \\
% --------------------------------------------------------------------------------------
...
\\ \\
\revised{The sentence was chnaged to "... $\mathcal{F}$ is the neutron generation operator given by \emph{the second term in} $q_{i,g}$ (see Eqs. 25-26)". The explicit form of the operators is given in Eqs. 25 and 26. }
\\ \\
...
\\ \\
\revised{The extrapolated distance for vacuum is set to $\zeta=2.13D$ (according to Lamarsh 1966). The Marshak boundary conditions are reproduced by $\zeta=2D$). The values assumed for the calculations are 1.3367 and 0.5459 cm for the fast and thermal groups, respectively. \\ \\
As for the need to update the extrapolated distance, it should be noted that the diffusion coefficient remains constant throughout the calculation, and so is the extrapolated distance. Only $\delta D$ is recalculated. Note that the correction in our numerical scheme is not by $D = D_0 + \delta D$ nor by $D = \delta D$. The correction of the diffusion coefficient is actually implemented through the currents, i.e., $J_{tr} = J_D + \delta J$.   }
\\ \\
% --------------------------------------------------------------------------------------
