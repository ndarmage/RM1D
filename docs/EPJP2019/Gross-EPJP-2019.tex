...
\end{align}
\end{subequations}
where integer and rational subscripts indicate node-averaged and interface quantities, respectively. The discretized form of the current $\delj$ must involve the neighboring flux as well, but its representation is changed into a drift-advection term to get rid of possible undefined division by zeros in case of flat flux~\cite{Smith-1983,Tomatis-2011}. The so-called ``transport current'' $\jtrr(x_s,E)$ is evaluated using the integral expression derived in~\sxn{sec:int-flux-J} as described in~\sxn{sec:calc-int-curr}.
...
\begin{enumerate}
	...
	\item\label{it:Js} Calculate currents - using the diffusion flux $\phi^{(k)}$, both the diffusion current $\jd^{(k)}$ (Eq.~\ref{eq:JD-dJ}) and the transport current $\jtrr^{(k)}$ (Eq.~\ref{eq:tot-J}) are calculated.
	\item\label{it:dD} Calculate correction factors - the correction factors $\delta D^{(k+1)}$ (Eq.~\ref{eq:JD-dJ}) is obtained using $\delj^{(k)}$ and Eq.~\eqref{eq:JD-dJ}.
	\item\label{it:dA} Update diffusion operator - reconstruct the diffusion operator $\mathcal{A}^{(k+1)} = \mathcal{A}_0+\delta \mathcal{A}^{(k+1)}$ (Eq.~\ref{eq:FD_diff_org}).
	\item\label{it:DF-solve} Diffusion solver - a one-dimensional multigroup diffusion solver is executed until convergence. The results are an updated estimation of the diffusion flux $\phi^{(k+1)}$ and the multiplication factor $\keff^{(k+1)}$.
...
% Place nodes
	...
	\node [block, below of=start, xshift=0cm,yshift=1.5cm]  (input) {Input\\parameters};
	\node [block, below of=input, xshift=0cm,yshift=1.5cm]  (geoXS) {Geometry, XS};
	\node [block, below of=geoXS, xshift=0cm,yshift=1.5cm]  (J)     {Calculate $\jtrr^{(k)},\jd^{(k)},\delj^{(k)}$};
	\node [block, below of=J,     xshift=0cm,yshift=1.5cm]  (dDA)   {Obtain $\delta D^{(k+1)}$, $\delta \mathcal{A}^{(k+1)}$};
	\node [block, below of=dDA,  xshift=0cm,yshift=1.5cm]  	(Anew)  {Construct\\$\mathcal{A}_0+\delta \mathcal{A}^{(k+1)}$};
	\node [block, below of=Anew, xshift=0cm,yshift=1.5cm]  	(Diff)  {Diffusion\\solver};
	\node [decision, below of=Diff,xshift=0cm,yshift=-1cm]	(cvg)   {$\phi^{(k+1)}$, $k^{(k+1)}$ \\ converged?};
	\node [cloud, below of=cvg,xshift=0cm,yshift=1.5cm] 	(stp)  {STOP};

% Draw edges
	\path [line] (start) -- (input);
	\path [line] (input) -- (geoXS);
	\path [line] (geoXS) -- node[anchor= south west,yshift=-0.2cm]{$\phi^{(0)}$, $k^{(0)}$, $\mathcal{A}_0$}(J);
	\path [line] (J) 	 -- (dDA);
	\path [line] (dDA) 	 -- (Anew);
	\path [line] (Anew)  -- node[near end,anchor= south west,xshift=0cm]{$\mathcal{A}^{(k+1)}$}(Diff);
	\path [line] (Diff)  -- node[anchor= south west,yshift=-0.2cm]{$\phi^{(k+1)},k^{(k+1)}$}(cvg);
	\path [line] (cvg)   -- node[anchor= south west]{YES}(stp);
	\path [line] (cvg)   -| node[near start,yshift=0.2cm]{NO}(-3,-6.0) -- node[near end]{}(J);
...
Table~\ref{tab:dx_drho} shows the reactivity differences as a function of mesh refinement using standard diffusion (with extrapolated length) and the Ronen method. The reactivity difference is given by $(\Delta\rho = 1/\kref - 1/k_\texttt{D/RM})\times 10^5$, where $\kd$ and $\krm$ are the multiplication factors of standard diffusion and of the Ronen method, respectively. The reference $\kref$ value from S\tsub{16} is 0.744417 ($I = 400$).  
...
\begin{table}[htbp]
	...
	\begin{tabular}{lcccccr}
		%\midrule
		$dx$[cm] & $I$ &  $\kref$ & $\kd$ & $\Delta\rho_\texttt{D}$ [pcm] & $\krm$ & $\Delta\rho_\texttt{RM}$ [pcm]\\ 
		\midrule
