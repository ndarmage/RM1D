...
Two one-dimensional test cases are considered, homogeneous and heterogeneous. The reference solutions are calculated using a Python3 discrete ordinates ($S_N$) code with $N=16$~\cite{Lewis-1984}. The diffusion solutions and the Ronen iterations are produced using original codes developed in Pyhton3. All thresholds are set to 1E-6 and no (over/under)-relaxation is done. The initial diffusion coefficient is set to $D=(3\sigma_{tr})^{-1}=(3\sigma)^{-1}$. Void boundary conditions are used (extrapolated distance) at the edges of the slab and they are constant for each energy group.
...
%\begin{figure}[htbp!]
%	\centering
%	\includegraphics[width=0.55\linewidth]{k_over_i_C3_10RMit.pdf}
%	\caption{Convergence of $\keff$ over number of grid points along core no. 3, for 100 RM iterations, and standard diffusion calculation.}
%	\label{fig:k_vs_I}
%\end{figure}
...
%\begin{figure}[htbp!]
%	\centering
%	\includegraphics[width=0.45\linewidth]{RM_flx_100it_8_22_728.pdf}
%	\caption{Fluxes as calculated by RM scheme. Results shown here after 100 Ronen iterations and 728 mesh points.}
%	\label{fig:Cores_RMflx}
%\end{figure}
