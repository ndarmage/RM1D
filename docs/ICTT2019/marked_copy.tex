...
%DIF DEL RMethod_ictt26.tex           Mon Aug 31 11:40:43 2020
%DIF ADD RMethod_ictt26_revised.tex   Tue Sep  1 17:14:03 2020
%
...
\newcommand{\shortTitle}      % Short title here
   {The Ronen Method in simple 1-D geometries}
...
% \begin{linenumbers}, end it with \end{linenumbers}. Or switch it
% on for the whole article with \linenumbers.
...
DES, Service d'{\'e}tudes des r\'eacteurs et de math\'ematiques appliqu\'ees (SERMA),\\
...
\end{equation}
where the subscript $l$ refers to the moments of the expansions of the flux and of the scattering cross section on the Legendre polynomials $P_l$. Fission emission is isotropic, so that only the scalar flux $\phi$ being equal to the first moment $\varphi_0$ is retained for the fission term. Without the external source $S$, the multiplication factor $k$ is used as eigenvalue to avoid the only trivial vanishing solution. Since $q$ is computed by the results of the diffusion equation in the Ronen method, \DIFaddbegin \DIFadd{the degree }\DIFaddend $l$ can only assume the values 0 and 1, with the second moment $\varphi_1$ equal to the net current $J$. \DIFaddbegin \DIFadd{Therefore, the sum over $l$ will always be truncated in the following. }\DIFaddend % Hereafter we consider only isotropic sources.
...
%DIFDELCMD < J^+(x) &\pluseq \frac{5}{4} \tilde{\phi}_2(a) \left[ 3 E_5(\tau(a, x)) - E_3(\tau(a, x)) \right],\\ &\text{ with } \tilde{\phi}_2(a) = - \frac{16}{5} \left( \frac{1}{4} \phi_0(a) + J^- \right).
%DIFDELCMD < \end{split}%%%
...
\[ \phi(a, \mu) = \frac{1}{2} \DIFdelbegin \DIFdel{\phi}\DIFdelend \DIFaddbegin \DIFadd{\varphi}\DIFaddend _0 + \frac{3}{2} \mu \DIFdelbegin \DIFdel{\phi}\DIFdelend \DIFaddbegin \DIFadd{\varphi}\DIFaddend _1 + \frac{5}{4} \left( 3 \mu^2 - 1 \right) \DIFdelbegin %DIFDELCMD < \tilde{\phi}%%%
...
%We resume here briefly the results of the two-group slab problem from \cite{tomatis2011application}. New results are also available for having new computer codes implemented in Python v3.8.2 with the numerical libraries NumPy v1.18.2 and SciPy v1.3.2. The reference solution is provided by a S$_{16}$ discrete ordinate computer code. Tab. \ref{tab:kconv} shows the differences on the reactivity by refining the mesh when the Ronen method is called with the diffusion solver. The reactivity is simply given as $\rho = 1 - 1/k$, where $k$ is the multiplication factor. The reference $k$ value from S$_{16}$ is 0.744417 ($I = 200$). Differences at $it=0$ correspond to the initial diffusion calculations without any current correction. Equidistant meshes are used in all calculations.
%
%\begin{table}[htbp]
%\centering
...
\]
The multiplicity $\nu$ recovers the role of eigenvalue in absence of external sources. The size of the problem is simply $b=r_c$ with $a=0$. %$c$ and $b$ are then the free parameters in the problem.
The material specifications of the benchmark cases are reported in Table \ref{tab:xs}. The naming convention of the test cases follows by Sood et al., with the critical lengths listed in Table \ref{tab:rc}. All problems use the vacuum boundary condition. %Problem PUa-1-0-SL reproduces the full slab, while PUb-1-0-SL is calculated as half slab, to study the artificial correction introduced on the second flux moment.
...
\DIFdelbeginFL \DIFdelFL{H20 (refl)}%DIFDELCMD < & %%%
\DIFdelFL{0.0 }%DIFDELCMD < & %%%
\DIFdelFL{0.0 }%DIFDELCMD < & %%%
\DIFdelFL{0.032640 }%DIFDELCMD < & %%%
\DIFdelFL{0.293760 }%DIFDELCMD < & %%%
\DIFdelFL{0.32640 }%DIFDELCMD < & %%%
\DIFdelFL{0.90}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelendFL %DIF >   H20 (refl)& 0.0 & 0.0 & 0.032640 & 0.293760 & 0.32640 & 0.90\\
\end{tabular}
\end{table}
...
% PUb-1-0-SL & Slab     & 0.9701734 & 0.8810540  & 0.7318131 &  0.4902592\\
...
%PUb-1-0-SP & Sphere   & 0.93538006& 0.75575352 & 0.49884364 & 0.19222603
\end{tabular}
\end{table}

After computing the numerical solution of the diffusion problem by finite differences without corrections at the cell interfaces, we evaluate new current estimates by using escape, transmission and reflection probabilities according to the given geometry type, as derived from the integral equation in section \ref{sec:ce}. Then, the solution by the Ronen method is sought iteratively as explained in section \ref{sec:impl}. \DIFaddbegin \DIFadd{Four integration points are used in the Gauss-Jacobi quadratures, yielding an error lower than 0.1\%. The solution by diffusion in these particular test cases is very different from the one by transport, which provides the physical solution. }\DIFaddend Anderson acceleration through the DAAREM algorithm (Damped Anderson Acceleration with Epsilon Monotonicity) is also used during the iteration process \cite{henderson2019damped}, reducing considerably the number of iterations to achieve residuals on the relative flux differences below $10^{-6}$. Its use is crucial to find the fixed point solution throughout the non-linear iterations of the RM. \DIFaddbegin \DIFadd{Very low convergence rates have been noticed without DAAREM, potentially leading to detect false convergence after a very high number of iterations, which means obtaining the wrong flux solution afterwards.%DIF >  that does not satisfy the neutron balance equation.
}\DIFaddend 

The calculation of half slab by CPM after setting (white) reflection at the center yields poor results due to missing contributions from the higher flux moments, see Figure \ref{fig:cpm_hslab_err}. \DIFaddbegin \DIFadd{Equidistant meshes are used by refining gradually the cell width $\Delta$. }\DIFaddend The solution obtained by RM takes into account \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend artificial correction applied to the only second moment, reducing the flux error \DIFaddbegin \DIFadd{(see the green curves in the left plot)}\DIFaddend , but its eigenvalue solution is higher than 1 of 836 pcm on converged meshes. \DIFaddbegin \DIFadd{The eigenvalue computed by CPM on half slab is lower than 1 of about 3000 pcm (see the right plot). }\DIFaddend This issue, discussed in section \ref{sec:bc}, is more severe with very thin slab, like in the case PUa-1-0-SL, and disappears when the slab thickness \DIFaddbegin \DIFadd{$r_c$ }\DIFaddend increases beyond a few mean free paths (mfp). This forces the modelling of the full slab when verifying the flux distribution for the case PUb-1-0-SL. Such issue does not appear at the center of the solid cylinder or sphere. % solid = not hollow
All methods computes a critical configuration for PUa-1-0-SL after unfolding the half-slab. The study to verify the fundamental flux is then \DIFdelbegin \DIFdel{only shown }\DIFdelend \DIFaddbegin \DIFadd{shown only }\DIFaddend for the case PUb-1-0-SL.

\begin{figure}[htbp]
\centering
\includegraphics[width=.8\linewidth]{cpm_hslab_err.pdf}
\caption{Error in the half slab (PUa-1-0-SL) with white reflection at the right and vacuum at the left\DIFaddbeginFL \DIFaddFL{; HS and FS indicate half slab and full slab, respectively}\DIFaddendFL .}
\label{fig:cpm_hslab_err}
\end{figure}

About the cases PUb, the relative error on the critical flux computed by CPM against the values from Table \ref{tab:rff} are shown in Figure \ref{fig:cpm_PUb_flxerr} \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend increasing number of cells ($I$) \DIFdelbegin \DIFdel{always using }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend equivolume meshes. The same figure shows also the results obtained by RM. \DIFaddbegin \DIFadd{Basically, RM and CPM results overlap but at the vacuum boundary where the monotonic behavior of the error trend is broken with fine meshes, leaving a residual error of about 1-2\% in the slab and in the cylinder, and higher than 3\% in the sphere. }\DIFaddend The spherical geometry needs finer meshes to decrease the error, which is \DIFdelbegin \DIFdel{always }\DIFdelend \DIFaddbegin \DIFadd{still }\DIFaddend higher near the \DIFdelbegin \DIFdel{vacuum }\DIFdelend \DIFaddbegin \DIFadd{outer }\DIFaddend boundary. Refined meshes near the boundary, drawn for instance by a geometric progression with ratio less than one, can lower this error, see the program released with this publication. The \DIFdelbegin \DIFdel{RM results practically overlaps those obtained by the CPM, but at the vacuum boundary where the monotonic behavior of the error trend is broken with fine meshes, leaving a residual error of about 1-2\% in the slab and in the cylinder, and higher than 3\% in the sphere. The }\DIFdelend flux at the boundary used to reproduce the current by diffusion is approximated by a quadratic polynomial fitting the midpoint fluxes at the nearest three cells, instead of the only flux of the last cell.

The \DIFaddbegin \DIFadd{deviation of the multiplication factor from unity by refining the mesh is shown in Figure \ref{fig:cpm_PUb_kerr}. When unfolding the reflected slab, the }\DIFaddend number of cells in the slab is doubled indeed for taking a thickness of $2r_c$ in the calculations. \DIFdelbegin \DIFdel{The deviation of the multiplication factor from unity by refining the mesh is shown in Figure \ref{fig:cpm_PUb_kerr}, instead}\DIFdelend \DIFaddbegin \DIFadd{This explains why the plotted error for the slab is higher than the one for the cylinder at a given value of $I$}\DIFaddend . The same trend for the three geometries is noticed, with the spherical one needing finer meshes to achieve the same level of error. The multiplication factor computed by RM is about the same as the one given by CPM.
\DIFdelbegin \DIFdel{Four integration points are used in the Gauss-Jacobi quadratures, yielding an error lower than 0.1\%.
}\DIFdelend 

\DIFdelbegin %DIFDELCMD < \begin{figure}[hp]
%DIFDELCMD < %%%
...
The solution by CPM needs a single inversion of a full matrix with the direct \DIFdelbegin \DIFdel{solving scheme from }\DIFdelend \DIFaddbegin \DIFadd{and fast solving scheme reported in }\DIFaddend appendix \ref{sec:CPMsolution}. This scheme may not be suitable to problems with a high number of energy groups and spatial cells. The RM needs to invert several times a tridiagonal matrix, which is efficiently performed in our code by \DIFaddbegin \DIFadd{the }\DIFaddend Thomas TDMA algorithm, a simplified form of Gaussian elimination that can be used to solve tridiagonal systems of equations \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{quarteroniNM2007}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Both methods share the calculation part of the first flight escape \DIFdelbegin \DIFdel{/}\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend collision probabilities. Typical runtimes of the benchmark problems treated in this article are quite fast \DIFaddbegin \DIFadd{(a few seconds or minutes depending on the size of the problem), }\DIFaddend and they can be \DIFaddbegin \DIFadd{easily }\DIFaddend run on any laptop \DIFdelbegin \DIFdel{computers}\DIFdelend \DIFaddbegin \DIFadd{computer}\DIFaddend . The calculation of the first flight probabilities is the most expensive part, especially in curvilinear geometries, and it should be reprogrammed in a compiled language to save runtime.


\section{CONCLUSION}
\label{sec:cncls}

\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Initially the }\DIFaddend Ronen Method was \DIFdelbegin \DIFdel{firstly }\DIFdelend proposed for better estimates of the diffusion coefficient by calculating the current with a higher-order transport operator and a known best-estimate neutron flux \DIFdelbegin \DIFdel{. This originates }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ronen2004accurate}}\hspace{0pt}%DIFAUXCMD
. This can originate }\DIFaddend an iterative scheme leading to new flux distributions solved by a diffusion solver yet with the aim to fulfill the integral transport equation. The direct resolution of the integral equation\DIFaddbegin \DIFadd{, as in the collision probability method, }\DIFaddend implies the inversion of full matrices, with poor control of their conditioning. The solution of the diffusion equation offers many numerical advantages instead, solution speed and robustness above all.

\DIFdelbegin \DIFdel{Nonetheless}\DIFdelend \DIFaddbegin \DIFadd{Furthermore}\DIFaddend , the same current \DIFaddbegin \DIFadd{obtained by the higher-order operator }\DIFaddend can be enforced in the discretized form of the diffusion equation as suggested by the CMFD \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{scheme, which }\DIFaddend avoids the numerical issues arising with flat flux. This is \DIFaddbegin \DIFadd{also }\DIFaddend the option adopted in this work. \DIFaddbegin \DIFadd{In particular, the RM is here developed under the common formalism of CPM theory, thus offering a formal derivation in the framework of integral transport with the nonlinear iterations of the CMFD scheme. Only first flight escape probabilities are computed, by which all other kinds of probabilities are derived using conservation and reciprocity properties.
}

\DIFaddend The implementation of the boundary conditions is improved to reproduce the expected vanishing current in case of reflection. However, this correction is not sufficient to \DIFdelbegin \DIFdel{fix }\DIFdelend \DIFaddbegin \DIFadd{remove }\DIFaddend the limitations due to the use of white reflection, which is particularly serious in problems smaller than a mean free path.

The \DIFdelbegin \DIFdel{RM is developed in this work by following the CPM formalism and theory. Only first flight escape probabilities are computed, by which all other kinds of probabilities are derived using conservation and reciprocity properties. The }\DIFdelend solutions are verified with simple one-group homogeneous problems \DIFaddbegin \DIFadd{and isotropic sources }\DIFaddend from the benchmark test suite by Sood and al. \cite{sood2003analytical}. More benchmark problems will be analyzed in a future work to study the RM with material heterogeneity and with many energy groups. The application of the RM to problems with anisotropic scattering suffers from the lack of the flux moments higher than the first, since diffusion can count only on the net current. This article shows the extension of the RM to linearly anisotropic scattering in the source terms in the case of the slab. The study of anisotropy still needs further developments.

The use of Anderson acceleration \DIFaddbegin \DIFadd{implemented }\DIFaddend in the RM iterations \DIFaddbegin \DIFadd{with the DAAREM algorithm }\DIFaddend was crucial to find converged results, fixing the issue of slow convergence noticed in the previous published works \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{tomatis2011application,gross2020high}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The large discrepancy on the scalar flux observed in previous works at bare vacuum boundaries has now been reduced to a few percent with respect to the reference solutions. \DIFdelbegin \DIFdel{This suggests that }\DIFdelend \DIFaddbegin \DIFadd{However, it is still necessary to investigate these remaining differences at the boundary, like using for instance higher-order numerical approximations of the discretized equations. This strong improvement in the flux behavior suggests that the }\DIFaddend same test problems could have been affected in the past implementations by false convergence for very slowly decreasing convergence rates. Additional tests are on-going to verify this assertion. The upgrade of the RM to coarser meshes and different implementations of the CMFD scheme will be the objects of future research and development. These are considered as central topics for the advancement of the methodology in practical applications.
...
The theory and results of this article were produced by the computer program available at \texttt{https://github.com/ndarmage/RM1D.git}. This program is \DIFaddbegin \DIFadd{freely }\DIFaddend distributed for educational purposes and basic training in transport theory, without containing any confidential material on nuclear data.
...
\label{eq:albedo_b}
 % J^- = \beta J^+ = \beta \left(
 %     \sum_{j=1}^I {\frac{q_j p_{jb}}{4}} + \tilde{t} J^-
 %   \right),
...
%\[
%J_b^- = \beta_b \left[
%      \sum_{j=1}^I {\frac{q_j p_{jb}}{4}} + \tilde{t} \beta_a \left(
%      \sum_{j=1}^I {\frac{q_j p_{ja}}{4}} + \tilde{t} J_b^- \right)
%    \right]
%\]
