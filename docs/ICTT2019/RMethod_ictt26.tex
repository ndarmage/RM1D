...
%\section{NODAL DIFFUSION}
%\label{sec:nd}
%
%Daniele: this topic is postponed to future works.
%
...
\label{eq:NTE}
\end{equation}
with the optical length $\tau(x_1, x_2) = \int_{x_1}^{x_2}{dx' \sigma_t(x')}$. Contrary to other formulations, we do not include $\mu$ in the definition of the optical length to ease the numerical integrations in the following. $x_0 = x_{-1/2} = a$ if $\mu > 0$, $x_0 = x_{I-1/2} = b$ otherwise. The inclusion of the energy variable in Eq. \ref{eq:NTE} is straightforward; we use hereafter the multi-group theory, writing the source $q_g$ as\footnote{Curly brackets remind the possible use of the $k$-eigenvalue in case of vanishing external sources.}:
\begin{equation*}
q_g = \sum_{g'=1}^G{\left[
...
\end{equation*}
where the subscript $l$ refers to the moments of the expansions of the flux and of the scattering cross section on the Legendre polynomials $P_l$. Fission emission is isotropic, so that the scalar flux $\phi$ is equal to the first moment $\varphi_0$. Without the external source $S$, the multiplication factor $k$ is used as eigenvalue to avoid the trivial vanishing solution. Since $q$ is computed by the results of the diffusion equation in the Ronen method, $l$ can only assume the values 0 and 1. Hereafter we consider only isotropic sources.
...
\end{aligned}
\end{equation*}
These quantities can be computed only for $i > j$ since the others are simply opposite in sign.

{\color{red}Roy-Erez: Please add a paragraph or reformulate the previous one to introduce the escape probabilities in the slab.}
...
\end{equation}
for $i<j$, where $\varepsilon^-_{i+1/2,j}\equiv 0$ for $i\geq j$ since in this case a neutron born in volume $j$ (or in volume $j=i$) cannot enter volume $i$ through its outer surface at radius $r_{i+1/2}$ without undergoing a collision. 

Singularity of the integrand for the $h$-integration is noticed in the convex part while computing the collision probabilities \cite{hebert2009applied}; this notably happens at the outer radius of $j$-th ring where $\hat{n}$ is perpendicular to $\mathbf{\Omega}_p$. However, that integral can be regularized by a suitable change of variable. % $\varrho = \sqrt{r - h}$.
...
After all escape and inscape probabilities are known, the first flight collision probabilities can be determined by conservation properties in order to reduce the amount of total operations. For instance, the probability for a neutron emitted isotropically in the region $i$ to collide in the same region must fulfill the following relation:
\begin{subequations}
\begin{equation}
p_{ii} + e^+_{i+1/2,i} + e^-_{i-1/2,i} = 1 \; \forall i.
\label{eq:pii}
\end{equation}
This can be generalized with neutrons coming from region $j$ and colliding in region $i$ as:
\begin{equation}
p_{ij} = e^+_{i-1/2,j} - e^+_{i+1/2,j} + e^-_{i+1/2,j} - e^-_{i-1/2,j},
\label{eq:pij}
\end{equation}
\label{eq:cps}
\end{subequations}
with $e^+_{i-1/2,i} = e^-_{i+1/2,i} = 0$ to find again Eq. \ref{eq:pii}.

Particle conservation on the full domain requires $\sum_i p_{ij} + e^+_{I+1/2,j} = 1$, $\forall j$. At last, reciprocity implies
\[
  \frac{p_{ij}}{V_i \sigma_i} = \frac{p_{ji}}{V_j \sigma_j} \quad \text{and} \quad
  \frac{p_{j, i+1/2}}{V_j \sigma_j} = \frac{4 e_{i+1/2, j}}{S_{i+1/2}} \quad \text{for } 1 \leq i,j, \leq I,
\]
with the escape probability counting for both contributions from partial currents. These expressions can be used to verify Eqs. \ref{eq:cps}.

Transmission probabilities between surfaces are also needed with boundary conditions different from vacuum. We do not derive them in this work because we only treat bare problems.
