...
\begin{equation}
\varphi(\mathbf{r}, \mathbf{\Omega}) = \int_0^S{ds\, \frac{q(\mathbf{r} - s \mathbf{\Omega}_p, \mathbf{\Omega})}{\sin \theta} \exp(-\frac{\tau(\mathbf{r}, \mathbf{r} - s \mathbf{\Omega}_p)}{\sin \theta})}. % + \varphi(\mathbf{r} - S \mathbf{\Omega}_p, \mathbf{\Omega}) \exp(-\frac{\tau(\mathbf{r}, \mathbf{r} - S' \mathbf{\Omega})}{\sin \theta}).
\label{eq:ITE_xy}
\end{equation}
...
Results in the cylindrical and spherical coordinate systems will be presented in the final version of the article.
